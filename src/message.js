...
async function Solving(RAEHAN2GD, store) {
	...
	RAEHAN2GD.sendGroupInviteV4 = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const message = generateWAMessageFromContent(participant, msg, { userJid: RAEHAN2GD.user.id, ...options }); 
	...
	RAEHAN2GD.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		}, { quoted: options?.quoted || null, userJid: RAEHAN2GD.user.id }); 
	...
	RAEHAN2GD.sendListMsg = async (jid, text = '', footer = '', title = '', subtitle = '', buttons = [], messageParamsJson = {}, media, contextInfo = {}, options = {}) => {
				...
				message: {
					...
					interactiveMessage: proto.Message.InteractiveMessage.create({
						...
						header: proto.Message.InteractiveMessage.Header.create({
							title: title || '',
							subtitle: subtitle || '',
							hasMediaAttachment: media && Object.keys(media).length > 0 ? true : false,
			...
			const _query = await RAEHAN2GD.query({
				...
				content: [{
					...
					content: new TextEncoder().encode(JSON.stringify({
						variables: /(FOLLOW|UNFOLLOW|DELETE)/.test(type) ? { newsletter_id } : type == 'CREATE' ? { newsletter_input: { name, description, picture }} : { fetch_creation_time: true, fetch_full_image: true, fetch_viewer_metadata: false,
						}
					}))
				}]
			},timeout);
