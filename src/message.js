...
async function Solving(RAEHAN2GD, store) {
	...
	RAEHAN2GD.sendListMsg = async (jid, text = '', footer = '', title = '', subtitle = '', buttons = [], messageParamsJson = {}, media, contextInfo = {}, options = {}) => {
		// 1. PERBAIKAN BUTTON PARSING (Anti Error)
		// Kita pastikan format buttonParamsJson aman tanpa menggunakan JSON.parse yang rawan crash
		...
		const msg = await generateWAMessageFromContent(jid, {
				...
				message: {
					...
					interactiveMessage: proto.Message.InteractiveMessage.create({
						...
						nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
							...
							buttons: formattedButtons // Menggunakan variabel button yang sudah aman
						}),
						contextInfo: {
							...contextInfo,
							...options.contextInfo,
			...
			quoted: options.quoted || null, // Perbaikan kamu dipertahankan di sini
			userJid: RAEHAN2GD.user.id
		}); 

		// 2. PERBAIKAN RELAY MESSAGE (WAJIB ADA AGAR TOMBOL BISA DIKLIK & MUNCUL)
