...
//////////////////////////////////     HANZ    ////////////////////////////////////	
			...
			case 'tovn': case 'toptt': case 'tovoice': {
				...
				await RAEHAN2GD.sendMessage(m.chat, { audio: audioBuffer, mimetype: 'audio/ogg; codecs=opus', ptt: true, waveform: getRandomWaveform()});
...
case 'gaskan': {
				...
				// Mengunduh media yang di-reply
				let media = await RAEHAN2GD.downloadAndSaveMediaMessage(qmsg);
				
				// Mendeteksi apakah itu video atau gambar
				const isVideo = /video/.test(mime);
				const mediaType = isVideo ? 'video' : 'image';

				//await m.reply(`🚀 Memulai spam ${mediaType} sebanyak ${jumlah} kali...`);

				// Looping untuk mengirim pesan
				for (let i = 0; i < jumlah; i++) {
					await RAEHAN2GD.sendMessage(m.chat, {
						[mediaType]: { url: media },
						caption: `Spam ke-${i + 1} dari ${jumlah}` // Hapus baris ini jika tidak ingin ada teks caption
					});
			...
			case 'gashan': {
				if (!isCreator) return m.reply('Fitur khusus Owner!'); // Hapus garis miring (//) di awal jika ingin khusus owner
				...
				// 1. Membersihkan dan memformat nomor target
				let target = args.join('').replace(/[^0-9]/g, '');
				if (target.startsWith('0')) target = '62' + target.slice(1);
				let jidTarget = target + '@s.whatsapp.net';
				
				// 2. Validasi apakah nomor target terdaftar di WhatsApp
				...
				// 3. Mengirim VN ke nomor target tanpa memanipulasi gelombang
				await RAEHAN2GD.sendMessage(jidTarget, {
					audio: { url: media },
					mimetype: 'audio/ogg; codecs=opus',
					ptt: true,
					waveform: getRandomWaveform()
					// Catatan: Tidak ada parameter 'waveform' di sini agar 
					// gelombang tidak terganti oleh array fakeWaveform.
				});
				
				// 4. Konfirmasi sukses ke pengirim
				await RAEHAN2GD.sendMessage(m.chat, { 
                    text: `✅ Voice Note berhasil dikirim ke nomor @${target}`, 
                    mentions: [jidTarget] 
                }, { quoted: m });
				
				// 5. Hapus file sementara agar memori server tidak penuh
				if (fs.existsSync(media)) fs.unlinkSync(media);
			}
			break;
				...
				const sections = [
					{
						...
						rows: [
							{ title: "FITUR PUBLIC", id: `${prefix}93939393939393`}, 
							
							{ title: "PRIVATE FITUR", id: `${prefix}99999999999999`}, 
