...
case 'buatkontak': {
				...
				let args = text.split(' ');
				let amount = 5; // Default jumlah kontak jika tidak diisi angka
				let customName = ''; // Default custom name

				// Logika pengecekan input
				// Jika argumen pertama adalah angka, jadikan sebagai jumlah (amount)
				if (args.length > 0 && !isNaN(parseInt(args[0]))) {
					amount = parseInt(args[0]);
					customName = args.slice(1).join(' '); // Sisanya jadikan nama
				} else {
					customName = text; // Jika tidak ada angka, seluruh teks jadikan nama
				}

				
				await sendLoading(m.chat, m);

				// Kumpulan kode negara
				const countryCodes = [
					'1', '20', '27', '30', '31', '32', '33', '34', '36', '39', '40', '41', '43', '44', '45', '46', '47', '48', '49', '51', '52', '53', '54', '55', '56', '57', '58', '60', '61', '62', '63', '64', '65', '66', '81', '82', '84', '86', '90', '91', '92', '93', '94', '95', '98', '212', '213', '216', '218', '220', '234', '244', '249', '254', '263', '267', '290', '299', '351', '352', '353', '354', '355', '358', '359', '370', '371', '372', '373', '374', '375', '380', '381', '385', '386', '387', '389', '420', '421', '501', '502', '503', '504', '505', '506', '507', '509', '591', '593', '595', '597', '598', '852', '853', '855', '856', '880', '886', '960', '961', '962', '963', '964', '965', '966', '967', '968', '971', '972', '973', '974', '975', '976', '977', '992', '993', '994', '995', '998'
				];

				let contactsList = [];
				
				for (let i = 0; i < amount; i++) {
					let randomCC = countryCodes[Math.floor(Math.random() * countryCodes.length)];
					let randomNum = Math.floor(Math.random() * 900000000) + 100000000; 
					let fullNumber = `${randomCC}${randomNum}`;
					
					// Logika Nama: Jika customName diisi, gunakan itu (tambah nomor urut agar tidak kembar persis namanya).
					// Jika dikosongkan, kembali ke default Random ID.
					let finalName = customName ? (amount > 1 ? `${customName} ${i + 1}` : customName) : `Random ID ${fullNumber.slice(-4)}`;

					let vcard = 'BEGIN:VCARD\n'
							  + 'VERSION:3.0\n'
							  + `FN:${finalName}\n`
							  + 'ORG:Random Internasional;\n'
							  + `TEL;type=CELL;type=VOICE;waid=${fullNumber}:+${fullNumber}\n`
							  + 'END:VCARD';
				...
				await RAEHAN2GD.sendMessage(m.chat, {
					contacts: {
						displayName: `${contactsList.length} Kontak Random`,
						contacts: contactsList
					}
				}, { quoted: fkontak });
				
				m.reply(`✅ Berhasil mengirim ${amount} kontak random dengan format nama: ${customName || 'Random ID'}!`);
			}
			break;
