...
async function gifToWebp(media) {
	...
	const tmpFileIn = isPath ? media : path.join(__dirname, '../HANZ-DATA', getRandomFile('gif'));
	const tmpFileOut = path.join(__dirname, '../HANZ-DATA', getRandomFile('webp'));
	try {
...
async function imageToWebp(media) {
	const isPath = typeof media === 'string';
	const tmpFileIn = isPath ? media : path.join(__dirname, '../HANZ-DATA', getRandomFile('jpg'));
	const tmpFileOut = path.join(__dirname, '../HANZ-DATA', getRandomFile('webp'));
	try {
		if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
		await new Promise((resolve, reject) => {
...
async function videoToWebp(media) {
	const isPath = typeof media === 'string';
	const tmpFileIn = isPath ? media : path.join(__dirname, '../HANZ-DATA', getRandomFile('mp4'));
	const tmpFileOut = path.join(__dirname, '../HANZ-DATA', getRandomFile('webp'));
	try {
		if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
		await new Promise((resolve, reject) => {
...
async function writeExif(media, data) {
	const isPath = typeof media === 'string';
	let tmpFileIn = isPath ? media : path.join(__dirname, '../HANZ-DATA', getRandomFile('tmp'));
	const tmpFileOut = path.join(__dirname, '../HANZ-DATA', getRandomFile('webp'));
	try {
		if (!isPath) await fs.promises.writeFile(tmpFileIn, media);
		...
		if (data) {
			...
			const { wra = data.pack_id ? data.pack_id : global.author ? global.author : 'DANCOK KOE', wrb = data.packname ? data.packname : global.packname ? global.packname : '+25777101400', wrc = data.author ? data.author : global.author ? global.author : 'UMAT KENTER', wrd = data.categories ? data.categories : [''], wre = data.isAvatar ? data.isAvatar : 0, ...wrf } = data;
